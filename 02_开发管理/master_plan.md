...
- **状态备注**：`completed`（plan `c2afaaa576107329ca6153a846fcb071c9383445`；implementation `4a44e99009e04bcbce5717df0a3073fffff9faf0`；PR #59 MERGED `43b6975a5dc4a92cde2f898acacd73a508831a48` mergedAt `2026-08-15T03:53:42Z`；CODE_REVIEW_APPROVED P0=0/P1=0；E2E 11 passed / contract 3 passed / ruff PASS / mypy src 0；zero `src/**`；feat 分支已删；`next_action=REL-001 planned / NOT AUTO-STARTED`；不得触碰 DEV-006/PR#13）。

#### REL-001 MVP RC Review 与验收清单

- **目标**：对照 `05_测试与验收/mvp_acceptance_checklist.md` A–F 与规格 §3.32 #1–#9 做 **RC Review 证据化**；仅在仓库证据满足后勾选清单；按 §4 分别处理 `v0.9.0-mvp-rc1`（E2E-001 与审查完成）与 `v1.0.0-mvp`（清单全部通过）。
- **非目标**：新业务功能；改 API/Schema/错误码/状态机；重测 STM/EXT/RET/CON/E2E-001 垂直切片；将 E2E 纳入 OPS-004 默认 CI；DEV-006/PR#13；真实计费 API / 真实 BGE-M3 作为 CPU MVP 阻塞；GPU；OpenTelemetry / 镜像签名 / 生产 Secrets Manager；改已执行 Migration 001–004；Agent `git tag`。
- **前置**：E2E-001 completed（PR #59 MERGED）— **SATISFIED**；OPS-001..004 completed；STM/EXT/RET/CON completed；baseline `main @ 412fb7b`。
- **规格章节**：`mvp_acceptance_checklist.md`；§3.32 #1–#9；交叉 §3.17 / §3.28 / §3.23 / §3.27 / §3.21 / §3.30。
- **预期生产变更**：NONE（`production_file_whitelist=NONE`；`test_file_whitelist=NONE`）。
...
| Task ID | Task | 规格章节 | 前置依赖 | 状态 |
|---|---|---|---|---|
| DEV-010 | SiliconFlow embedding token-estimation routing | §1.2.1, §2.1.13, §2.2.3, §2.2.5, §3.1, §3.10.0 | REL-001 completed; DEV-007; STM-001; OI-012 | reviewed |
...
- **状态备注**：`reviewed`（CODE_REVIEW_APPROVED session 93de8d64-bcaf-478c-8c57-f0c77c8e8670；P0=0 P1=0 P3=2；Commit Recorder READY_FOR_HUMAN_COMMIT session 44cb5320-381f-4ebe-95ac-b46b9f74c9ab；plan_commit `a55f99167863f508ef09033e13134348ab5e8b60`；implementation_commit=null until feat `git rev-parse`；unit+contract 18 passed；ruff PASS；mypy src 0；`next_action=IMPLEMENTATION_RELEASE`）。
...
|---|---|
| 日期 | 2026-08-15 |
| 原因 | Release Operator POST_MERGE_CLEANUP：PR #59 MERGED；docs(status): complete on main；exact feat 删除 |
| 受影响任务 | `E2E-001` → `completed`（计划文件 `02_开发管理/tasks/E2E-001-full-chain-e2e-failure-injection.md`）；`REL-001` 保持 `planned` / **NOT AUTO-STARTED**；**不**修改 OPS-004 完成状态；**不**触碰 DEV-006/PR #13 |
| 是否改变技术规格 | **否**（仅治理状态：completed + PR MERGED；不改 Contract/Schema） |
| 审批 | AFTER_PR_MERGE；PR #59 state=MERGED base=main head=`feat/E2E-001-full-chain-e2e-failure-injection` merge `43b6975a5dc4a92cde2f898acacd73a508831a48` mergedAt `2026-08-15T03:53:42Z` |
...
|---|---|
| 日期 | 2026-08-15 |
| 原因 | Planner：START_EXISTING_TASK REL-001 planning-only；新建 Task Plan；progress 规划态切到 REL-001 `planned` / `计划审查` |
| 受影响任务 | `REL-001` → `planned` / `AWAIT_PLAN_REVIEW`（计划文件 `02_开发管理/tasks/REL-001-mvp-rc-review-acceptance-checklist.md`）；**不**修改 E2E-001 completed 事实；**不**标 REL-001 approved/completed；**不**触碰 DEV-006/PR #13 |
| 是否改变技术规格 | **否**（仅治理规划态 + Task Plan；不改 Contract/Schema） |
| 审批 | 待独立 Plan Review；`human_plan_approved=false`；`developer_authorized=false` |
...
|---|---|
| 日期 | 2026-08-15 |
| 原因 | Human PLAN_APPROVED REL-001；Release Operator PLAN_LANDING：docs(plan) on main then create exact feat |
...
|---|---|
| 日期 | 2026-08-15 |
| 原因 | Developer：DEV-010 在 exact feat 上开始实施（approved → in_progress）；persist developer_authorized=true |
| 受影响任务 | `DEV-010` → `in_progress`（计划文件 `02_开发管理/tasks/DEV-010-siliconflow-embedding-token-estimation-routing.md`）；**不**修改 REL-001 completed 事实；**不**标 DEV-010 completed；**不**创建 OI-013；**不**触碰 DEV-006/PR #13 |
| 是否改变技术规格 | **是**（最小 provider-aware tokenize 计数来源澄清；不改 Schema/错误码/状态机） |
| 审批 | Human PLAN_APPROVED 2026-08-15 08:03 UTC；PLAN_LANDING plan_commit `a55f99167863f508ef09033e13134348ab5e8b60`；Developer 无 Git 写 |
...
|---|---|
| 日期 | 2026-08-15 |
| 原因 | Developer：DEV-010 实施完成并验证（in_progress → implemented → tested）；等待独立 Code Review |
| 受影响任务 | `DEV-010` → `tested`（计划文件 `02_开发管理/tasks/DEV-010-siliconflow-embedding-token-estimation-routing.md`）；**不**修改 REL-001 completed 事实；**不**标 DEV-010 committed/completed；**不**创建 OI-013；**不**触碰 DEV-006/PR #13 |
| 是否改变技术规格 | **是**（最小 provider-aware tokenize 计数来源澄清；不改 Schema/错误码/状态机） |
| 审批 | 待独立 Code Review；Developer 无 Git 写；unit+contract 18 passed；ruff PASS；mypy src 0 |
...
|---|---|
| 日期 | 2026-08-15 |
| 原因 | Code Reviewer + Commit Recorder：DEV-010 tested → reviewed；CODE_REVIEW_APPROVED P0=0 P1=0 P3=2；READY_FOR_HUMAN_COMMIT；implementation_commit 仍 null |
| 受影响任务 | `DEV-010` → `reviewed`（计划文件 `02_开发管理/tasks/DEV-010-siliconflow-embedding-token-estimation-routing.md`）；**不**修改 REL-001 completed 事实；**不**标 DEV-010 committed/completed；**不**创建 OI-013；**不**触碰 DEV-006/PR #13 |
| 是否改变技术规格 | **是**（最小 provider-aware tokenize 计数来源澄清；不改 Schema/错误码/状态机） |
| 审批 | CODE_REVIEW_APPROVED session 93de8d64-bcaf-478c-8c57-f0c77c8e8670；P0=0 P1=0 P3=2；Commit Recorder session 44cb5320-381f-4ebe-95ac-b46b9f74c9ab |
