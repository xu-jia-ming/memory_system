...
| DEV-OPS-006 | Phase 0 Baseline Hygiene Before STM-001 | 非业务：unit compose-wrapper allowlist + progress DOC_CODE_DRIFT hygiene | DEV-007, OI-011 | tested |
...
- **状态备注**：`tested`（exact-path allowlist + SHOULD_FIX 存在性断言；unit **216** / contract **47** / ruff / mypy 全绿；plan_commit `09b045be1429716eab184e4565beb30cf2856b28`；feat `feat/DEV-OPS-006-phase0-baseline-hygiene-before-stm001`）；等待独立 Code Review；未 commit。
...
| 受影响任务 | 新增 `DEV-OPS-006`（现 `tested`）；**不**实现 STM-001；**不**触碰 DEV-006/PR #13；**不**改 TEI 12g / SiliconFlow / compose*.yaml |
| 是否改变技术规格 | **否** |
| 审批 | Plan Reviewer PLAN_APPROVED（BLOCKER=0 MUST_FIX=0）；人工确认 PLAN_APPROVED；PLAN_LANDING 完成；Developer tested → 等待 Code Review |
