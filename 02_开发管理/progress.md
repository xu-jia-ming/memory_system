...
spec_version: 9
current_phase: Phase 1 — STM-008 tested
...
formal_STM-008_note: "实施完成（tested）；单 Lua Finalize：12 precondition + version bump + LTRIM + pending clear + compare-and-delete lock release；STM-007 CompressionFinalizeLlmPayload handoff；token 公式 §1000–1006（I18 new=500；I27 clamp 0）；畸形 Redis 整数→invalid_session_state；ARGV[11]==ARGV[7] defense；closing in-flight 允许；scoped unit 13 / contract 4 / integration 27；full unit 393 / contract 80；ruff PASS；mypy PASS；无 Kafka/Mongo/LLM；OI-004/OI-005 remain open；READY_FOR_CODE_REVIEW"
...
deferred_business_task_note: "STM-008 实施完成（tested）；integration 27 PASS；READY_FOR_CODE_REVIEW"
...
**下游**：**STM-008** `planned` — Task Plan 已创建；`next_action=计划审查`；**不得自动实施**；**STM-011** `READY_FOR_PLANNING only`（STM-006 satisfied）；**STM-009** NOT ready（needs STM-008）；**不得触碰 DEV-006/PR#13**。
...
1. **STM-008**：`planned` — Task Plan `02_开发管理/tasks/STM-008-compression-finalize-lua.md`；`next_action=计划审查`；**不得自动实施**；**不得触碰 DEV-006/PR#13**。
